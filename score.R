...
harmScoreObj <- function(v) {
  ...
  x$mod <- ourRollMax(2.5 + 2.5*(ourScale(v$spacial0) - median(ourScale(v$spacial0))),10*max(30,v$fps[1]))
...
pcaScore <- function(x, v) {
  ...
  pcas <- lapply(c(2:30), function(i) { ourScale( pca$scores[,i] ) })
  len <- length(x$time)
  for (t in c(1:len)) {
